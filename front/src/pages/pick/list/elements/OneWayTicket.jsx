...
export default function OneWayTicket({twoWay}){
    const [ticketType, setTicketType] = useState(true); //앞면<->뒷면
    const [contentType, setContentType] = useState(true); //앞면 개요<->상세
    const [compareChecked, setCompareChecked] = useState(false); //비교 체크 여부
    ...
    const changeCompareChecked = () => { //비교 체크
        if(twoWay){
