...
export default function OneWayTicket({twoWay, unifyChecked, isCheck, isLike, ticket, flightList}){

    const [ticketType, setTicketType] = useState(true); //앞면<->뒷면
    const [contentType, setContentType] = useState(true); //앞면 개요<->상세
    const [compareChecked, setCompareChecked] = useState(isCheck); //비교 체크 여부
    const [alarmChecked, setAlarmChecked] = useState(isLike);
    const [flightId, setFlightId] = useState(null);
    const dispatch = useDispatch();


    useEffect(() => {
        setAlarmChecked(isLike)
    }, [isLike]);
    ...
    const changeCompareChecked = (event) => { //비교 체크
        if(twoWay){
            console.log('부모에게 둘 다 적용하라고 보내줘...');
            unifyChecked('compare')
        }
        else{
            ...
            if(compareChecked){
                dispatch(compareAction.deleteCompareItem({
                    mode: twoWay? 'round' : 'oneWay',
                    flightId: twoWay?  '1234554321' : '12345',
                    // element: event.target.parentElement.parentElement.parentElement,
                }))
            }
            else{
                dispatch(compareAction.addCompareItem({
                    mode: twoWay? 'round' : 'oneWay',
                    flightId: twoWay?  '1234554321' : '12345',
                    // element: event.target.parentElement.parentElement.parentElement,
                }))
    ...
    const changeAlarmChecked = () =>{
        if(twoWay){
            ...
            setAlarmChecked((alarmChecked) => {
                if(alarmChecked) alert('알람이 해제되었습니다.');
    ...
    return(
        <Ticket ref={flightTicket} onAnimationEnd={() => deleteChangeAnimation(flightTicket)}>
            ...
            <TicketRight>
                <TicketRightTop>
                    <svg onClick={changeAlarmChecked} className="bell" width="28" height="32" viewBox="0 0 28 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <mask id="path-1-inside-1_197_7036" fill={alarmChecked? 'gold' : 'white'}>
                            <ellipse cx="13.9163" cy="2.78552" rx="2.51497" ry="2.78259"/>
                        </mask>
                        <ellipse cx="13.9163" cy="2.78552" rx="2.51497" ry="2.78259" fill={alarmChecked? 'gold' : 'white'}/>
                        ...
                        <path fillRule="evenodd" clipRule="evenodd" d="M14.084 2.64746C7.85974 2.64746 2.61127 7.28598 1.8462 13.463L1.80483 13.797C1.64912 15.0542 1.68697 16.2862 1.89371 17.4652H1.50898L0 27.8303H28L26.6587 17.4652H26.2743C26.481 16.2862 26.5188 15.0542 26.3631 13.797L26.3218 13.463C25.5567 7.28597 20.3082 2.64746 14.084 2.64746Z" fill={alarmChecked? 'gold' : 'white'}/>
