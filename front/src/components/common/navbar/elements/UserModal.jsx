...
import { getFirebasToken } from "../../../../apis/firebase";
...
export default function UserModal({ initialLoginMode, handleUserModalState }) {
  ...
  const [signupPasswordCheckErrMsg, setSignupPasswordCheckErrMsg] =
    useState("");
    ...
    sendImageToS3(profileImage)
      .then(async () => {
        ...
        try {
          ...
          dispatch(
            userAction.setAccessToken(loginResponse.headers.authorization)
          );
          handleUserModalState("button");
        } catch (error) {
          ...
          if (error.response.data.message === "동일한 아이디가 존재합니다.") {
            ...
            error.response.data.message === "동일한 닉네임이 이미 존재합니다."
          ) {
            nicknameInput.current.focus();
          }
        }
