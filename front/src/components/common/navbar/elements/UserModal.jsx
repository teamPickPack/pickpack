...
export default function UserModal({initialLoginMode, handleUserModalState}) {
    ...
    const doLogin = () => {
        ...
        console.log(data);
        axios.post('https://j8b307.p.ssafy.io/api/member/login', data,  {
        ...
        <UserModalBox>
            ...
            <UserModalContent>
                ...
                {loginMode ? <UserModalLogin>
                    <TextInputBox>
                        <FiUser size={30} />
                        <input onChange={handleLoginId} value={loginId} type='text' placeholder="아이디" />
                    </TextInputBox>
                    <ErrorMessage className="login-error"></ErrorMessage>
                    <TextInputBox>
                        <FiKey size={30}/>
                        <input onChange={handleLoginPassword} value={loginPassword} type='password' placeholder="비밀번호" />
                    </TextInputBox>
                    <ErrorMessage className="login-error">아이디 또는 비밀번호가 틀렸습니다.</ErrorMessage>
                    <LoginBtn onClick={doLogin}>
                        <span>로그인</span>
                    </LoginBtn>
                </UserModalLogin> : <div>
                    <div style={{display: 'flex', justifyContent: 'space-around', alignItems: 'center', border: '1px solid red', padding: '16px 0px 8px 0px'}}>
                        <ImageDiv>
                            <label htmlFor="profile-image-input">
                            <ProfileImage
                                src={profileImagePreview? profileImagePreview.url : defaultProfile}
                                alt="#"
                            />
                            </label>
                        ...
                        <div style={{border: '1px solid green'}}>
                            <div style={{display: 'flex'}}>
                                <TextInputBox inspect={true}>
                                    <FiUser size={30} />
                                    <input onChange={handleSignupId} value={signupId} className="with-inspect" type='text' placeholder="아이디" style={{width: '188px'}}/>
                                </TextInputBox>
                                    <div style={{height: '32px', border: '1px solid black', padding: '8px 16px', display: 'flex', justifyContent: 'center',alignItems: 'center', borderRadius: '0px 16px 16px 0px'}}>
                                        <span style={{fontSize: '16px'}}>검사</span>
                                    </div>
                            </div>
                            <ErrorMessage style={{textAlign: 'left', height: '8px', margin: '4px 0px 8px 0px'}}>아이디는 8자 이상 12자 이하만 가능합니다.</ErrorMessage>
                            <div style={{display: 'flex'}}>
                                <TextInputBox>
                                    <FiHash size={30} />
                            ...
                            <ErrorMessage style={{textAlign: 'left', height: '8px', margin: '4px 0px 8px 0px'}}>비밀번호는 8자 이상 12자 이하만 가능합니다.</ErrorMessage>
                            <div style={{display: 'flex'}}>
                                <TextInputBox>
                                    <FiKey size={30} />
                            ...
                            <ErrorMessage style={{textAlign: 'left', height: '8px', margin: '4px 0px 8px 0px'}}>비밀번호와 일치하지 않습니다.</ErrorMessage>
                        </div>
                    </div>
                    <div onClick={doSignup} style={{ margin: '0px auto',border: '1px solid black', width: '80px', fontSize: '16px', padding: '12px 16px', borderRadius: '8px', textAlign: 'center'}}>
                        <span>회원가입</span>
                    </div>
                </div> }
            </UserModalContent>
